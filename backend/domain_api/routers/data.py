...
async def fundamentals(ticker: str):
    income_coro  = _fmp_get(f"income-statement/{ticker}", {"period": "annual", "limit": "5"})
    metrics_coro = _fmp_get(f"key-metrics/{ticker}", {"limit": "5"})
    income_raw, metrics_raw = await asyncio.gather(income_coro, metrics_coro, return_exceptions=True)

    income = []
    if not isinstance(income_raw, Exception) and isinstance(income_raw, list):
    ...
    metrics = []
    if not isinstance(metrics_raw, Exception) and isinstance(metrics_raw, list):
