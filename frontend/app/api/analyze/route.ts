...
export async function POST(req: NextRequest) {
  ...
  // Fetch everything in parallel — individual failures are tolerated
  const [quoteRes, histRes, incomeRes, metricsRes, newsRes] =
    await Promise.allSettled([
      getQuote(ticker),
      getPriceHistory(ticker, "3M"),
  ...
  const income =
    incomeRes.status === "fulfilled" ? incomeRes.value[0] : null;
  const metrics =
    metricsRes.status === "fulfilled" ? metricsRes.value[0] : null;
...
DATI TECNICI (3 mesi):
- Prezzo: $${fmt(quote?.price)} | Var. 1G: ${fmt(quote?.day_change_percent)}% | Rend. 3M: ${fmt(ret3m)}%
...
- Market Cap: ${quote?.market_cap ? "$" + (quote.market_cap / 1e9).toFixed(1) + "B" : "N/A"}${income ? `
- Revenue (ultimo FY): ${income.revenue ? "$" + (income.revenue / 1e9).toFixed(2) + "B" : "N/A"} | Net Income: ${income.net_income ? "$" + (income.net_income / 1e9).toFixed(2) + "B" : "N/A"} | EPS: $${fmt(income.eps)}` : ""}`
      : "";
